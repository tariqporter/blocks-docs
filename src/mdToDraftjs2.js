...
const joinCodeBlocks = splitMd => {
  ...
  const opening = splitMd.findIndex(x => x.length >= 3 && x.substr(0, 3) === '```');
  const closing = splitMd.indexOf('```', opening + 1);
  

  if (opening >= 0 && closing >= 0) {
    const openingLine = splitMd[opening];
    const language = /```(js|html|css)/gi.exec(openingLine);
    ...
    if (language && language[1]) {
      // console.log(languageMap[language[1]]);
...
const parseMdLine = (line, existingEntities, extraStyles = {}) => {
  ...
  if (astString.children[0]) {
    ...
    if (astString.children[0].lang) lang = astString.children[0].lang;
  }
...
export function mdToDraftjs2(mdString, extraStyles) {
